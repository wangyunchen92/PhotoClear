...
- (void)cacheRow:(NSString *)str {
    NSArray *arr = [str componentsSeparatedByString:@","];
    if (arr.count >= 2) {
        [arr enumerateObjectsUsingBlock:^(NSString *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (idx == 0) {
                self.selectRowWithProvince = [obj integerValue];
                self.selectRowWithCity = 0;
                [self.pickerView reloadComponent:1];
                [self.pickerView selectRow:[obj integerValue] inComponent:0 animated:YES];
            }else if (idx==1)
            {
                self.selectRowWithCity = [obj integerValue];
                [self.pickerView reloadComponent:2];
                [self.pickerView selectRow:[obj integerValue] inComponent:1 animated:YES];
            }else{
                self.selectRowWithTown = [obj integerValue];
                [self.pickerView selectRow:[obj integerValue] inComponent:2 animated:NO];
            }
        }];
//        [self.pickerView selectRow:[[arr objectAtIndex:0] integerValue] inComponent:0  animated:NO];
//        [self.pickerView selectRow:[[arr objectAtIndex:1] integerValue]  inComponent:1 animated:NO];
//        [self.pickerView selectRow:[[arr objectAtIndex:2] integerValue] inComponent:2 animated:NO];
    }
...
- (void)determineBtnAction:(UIButton *)button
{
    ...
    if (self.determineBtnBlock) {
        self.determineBtnBlock(provincemodel.code, cityModel.code, areaModel.code, provincemodel.name, cityModel.name, areaModel.name, areaModel.postCode,[NSString stringWithFormat:@"%ld,%ld,%ld",(long)shengRow,(long)shiRow,(long)xianRow]);
    }
    [self dismiss];
}
